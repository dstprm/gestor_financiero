const APP_NAME = process.env.NEXT_PUBLIC_APP_NAME ?? 'ClaudeKit'
const APP_URL = process.env.NEXT_PUBLIC_APP_URL ?? 'http://localhost:3000'

export function inviteEmail(inviterName: string, orgName: string, inviteUrl: string) {
  return `<h2>You've been invited to join ${orgName} on ${APP_NAME}</h2>
...
export function inviteAcceptedEmail(newMemberName: string, memberEmail: string, orgName: string) {
  return `<p><strong>${newMemberName}</strong> (${memberEmail}) just joined <strong>${orgName}</strong> on ${APP_NAME}.</p>`
}
...
export async function sendInviteAcceptedEmail(
  ...
  return sendEmail({
    to: ownerEmail,
    subject: `${memberName} has joined your organization`,
...
export function trialExpiringEmail(userName: string, daysLeft: number) {
  return `<p>Your ${APP_NAME} trial ends in <strong>${daysLeft} day${daysLeft === 1 ? '' : 's'}</strong>. Upgrade to keep access.</p>
<p><a href="${APP_URL}/app/settings?tab=billing" style="display:inline-block;padding:12px 24px;background:#2563eb;color:#fff;text-decoration:none;border-radius:8px;font-weight:600;">Upgrade now</a></p>`
}
