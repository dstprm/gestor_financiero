...
import { ArrowLeft, Pencil, ExternalLink, LogOut, CreditCard } from 'lucide-react'
import Link from 'next/link'
import { cn } from '@/lib/utils'
import { isMPConfigured } from '@/lib/mercadopago'
import type { SubStatus } from '@/lib/subscription'

type Tab = 'organization' | 'profile' | 'billing'
...
export default function SettingsPage() {
  ...
  const activeTab: Tab = TABS.some((t) => t.id === rawTab) ? (rawTab as Tab) : 'billing'
  ...
  useEffect(() => {
    ...
    fetch('/api/org/mine')
      ...
      .then((d: { organizationId?: string; orgs?: OrgEntry[] } | null) => {
        const resolvedOrgId = !urlOrgId && d?.organizationId ? d.organizationId : (urlOrgId ?? null)
        ...
        if (resolvedOrgId) {
          fetch(`/api/subscription?organizationId=${encodeURIComponent(resolvedOrgId)}`)
            .then((r) => r.ok ? r.json() : null)
            .then((s: { status?: SubStatus; daysLeftInTrial?: number | null } | null) => {
              if (s?.status) setSubStatus(s.status)
              if (s?.daysLeftInTrial !== undefined) setDaysLeftInTrial(s.daysLeftInTrial ?? null)
            })
            .catch(() => {})
        }
      })
        ...
        {activeTab === 'billing' && (
          <div className="space-y-4">
            <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-5">
              ...
              {subStatus && (subStatus === 'active' || subStatus === 'past_due' || subStatus === 'paused') && (
                <div>
                  {isMPConfigured ? (
                    <>
                      <a
                        href="/upgrade"
                        className="w-full block text-center px-4 py-2.5 bg-gray-900 dark:bg-slate-700 hover:bg-gray-700 dark:hover:bg-slate-600 text-white text-sm font-medium rounded-xl transition-colors"
                      >
                        Manage Subscription →
                      </a>
                    ...
                    <p className="text-sm text-gray-400 dark:text-slate-500">
                      Configure Mercado Pago to manage your subscription.
                    </p>
                  )}
                </div>
              )}
