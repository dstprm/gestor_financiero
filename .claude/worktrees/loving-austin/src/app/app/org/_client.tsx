      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        {dbUnavailable && (
          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-8 text-center">
            <p className="text-slate-400 text-sm">Organization management requires database setup.</p>
            <p className="text-slate-500 text-xs mt-1">
              Configure <code className="bg-slate-700 px-1 rounded">DATABASE_URL</code> to enable this feature.
            </p>
          </div>
        )}
            ...
            <section className="bg-slate-900 border border-slate-800 rounded-xl p-5">
              ...
              {editingName ? (
                ...
                <div className="flex items-center gap-2 group">
                  <p className="text-slate-200 text-sm font-medium">{orgName ?? '—'}</p>
                  {isAdmin && (
                    <button
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-semibold text-slate-200">
                    Members
                    {members.length > 0 && (
                      <span className="ml-2 text-xs text-slate-500 font-normal">{members.length}</span>
                    )}
                  </h2>
                </div>
                {isAdmin && (
                    ...
                    {isAdmin && member.role !== 'OWNER' && (
                      confirmRemoveId === member.id ? (
                        ...
                        <button
                          ...
                          disabled={removingId === member.id}
                          className="p-1.5 text-slate-600 hover:text-red-400 hover:bg-red-400/10 rounded transition-colors disabled:opacity-40 shrink-0"
                          title="Remove member"
                        >
                          <Trash2 size={14} />
                        </button>
            ...
            <section className="space-y-3">
              ...
              {!isOwner && currentMembershipId && (
                <div className="bg-red-950/10 border border-red-900/30 rounded-xl p-5">
                  <h3 className="text-sm font-semibold text-red-400 mb-1">Leave organization</h3>
                  <p className="text-slate-400 text-sm mb-4">
                    You will lose access to{' '}
                    <span className="font-medium text-slate-300">{orgName ?? 'this organization'}</span>
                  ...
                  {confirmLeave ? (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 text-sm border border-slate-700 text-slate-400 hover:border-slate-500 hover:text-slate-200 rounded-lg transition-colors"
                      >
                        Cancel
                      </button>
                      <button
                        ...
                        disabled={leaving}
                        className="px-3 py-1.5 text-sm bg-red-600/20 hover:bg-red-600/30 text-red-400 rounded-lg border border-red-600/40 disabled:opacity-40 flex items-center gap-1.5 transition-colors"
                      >
                        <LogOut size={13} />
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 text-sm border border-red-600/40 text-red-400 hover:bg-red-950/30 rounded-lg transition-colors"
                    >
                      <LogOut size={13} />
                      Leave organization
                    </button>
              ...
              {isOwner && (
                <div className="bg-red-950/10 border border-red-900/30 rounded-xl p-5">
                  <h3 className="text-sm font-semibold text-red-400 mb-1">Delete organization</h3>
                  <p className="text-slate-400 text-sm mb-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className="block text-xs text-slate-500 mb-1">
                        Type <span className="font-mono text-slate-400">{orgName}</span> to confirm
                      </label>
                      <input
                        type="text"
