...
import { DEFAULT_PPTX_THEME, DEFAULT_DISPLAY_SETTINGS } from '@/types';
  ...
  persist(
    immer((set, get) => {
      ...
      return {
        ...
        createScenario(name, description = '', fromScenarioId) {
          ...
          if (orgId && typeof window !== 'undefined') {
            fetch('/api/scenarios', {
              ...
              body: JSON.stringify({ organizationId: orgId, name, description, employees: snap.employees }),
            })
              .then((r) => r.ok ? r.json() : null)
              .then((data: { id: string } | null) => {
                if (data?.id) {
                  set((s) => { if (s.scenarios[snap.id]) s.scenarios[snap.id].dbId = data.id; });
                }
              })
              .catch(() => null);
          }
        ...
        deleteScenario(id) {
          const dbId = get().scenarios[id]?.dbId;
          set((s) => {
          ...
          if (dbId && typeof window !== 'undefined') {
            fetch(`/api/scenarios/${dbId}`, { method: 'DELETE' }).catch(() => null);
          }
        },
        ...
        promoteToBaseline(id) {
          ...
          if (orgId && typeof window !== 'undefined') {
            // Write promoted scenario's employees to Employee table
            const employees = scenario?.employees ?? [];
            fetch('/api/employees', {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ organizationId: orgId, nodes: employees }),
            }).catch(() => null);
            // Remove from Scenario table (now lives in Employee table)
            if (scenario?.dbId) {
              fetch(`/api/scenarios/${scenario.dbId}`, { method: 'DELETE' }).catch(() => null);
              set((s) => { if (s.scenarios[id]) s.scenarios[id].dbId = null; });
            }
          }
        },
        ...
        cloneScenario(id, newName) {
          ...
          if (orgId && typeof window !== 'undefined') {
            fetch('/api/scenarios', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ organizationId: orgId, name: newName, description: `Cloned from ${source.name}`, employees: snap.employees }),
            })
              .then((r) => r.ok ? r.json() : null)
              .then((data: { id: string } | null) => {
                if (data?.id) {
        ...
        setLayoutDirection(dir) {
          ...
          set((s) => {
            s.savedPositions[currentDir] = currentPositions;
        ...
        setDisplaySettings(patch: Partial<DisplaySettings>) {
          // Use Object.assign to mutate the immer draft directly.
          // Spreading a draft proxy (`{ ...s.displaySettings }`) embeds child draft
          // proxies in the replacement object; those are revoked after finalization,
          // causing "Cannot perform 'get' on a proxy that has been revoked" on the
          // next render.
          set((s) => { Object.assign(s.displaySettings, patch); });
        },
        ...
        async saveDisplaySettings() {
          ...
          try {
            ...
            console.error('[SimplyOrg] Failed to save display settings', err);
          }
        },
        ...
        async loadFromDB(organizationId: string) {
          ...
          try {
            const [empRes, scenRes] = await Promise.all([
              fetch(`/api/employees?organizationId=${encodeURIComponent(organizationId)}`),
              fetch(`/api/scenarios?organizationId=${encodeURIComponent(organizationId)}`),
            ]);
            if (!empRes.ok) {
            ...
            const employees: Employee[] = rows.map((r) => ({
              ...
              location: '',
              status: (
                r.status === 'OPEN_ROLE' ? 'vacant' :
                r.status === 'INACTIVE' ? 'on-leave' :
                r.status === 'CONTRACTOR' ? 'contractor' :
                'active'
              ) as Employee['status'],
            ...
            // Baseline scenario (from Employee table)
            const baselineSnap = freshScenario('Current State', employees, 'Loaded from database', true);

            // Non-baseline scenarios (from Scenario table)
            const savedScenarios: Array<{
              id: string; name: string; description: string;
              employees: Employee[]; createdAt: string;
            }> = scenRes.ok ? await scenRes.json() : [];

            const additionalScenarios: Record<string, ScenarioSnapshot> = {};
            for (const dbScen of savedScenarios) {
              const snap: ScenarioSnapshot = {
                id: generateId(),
                dbId: dbScen.id,
                name: dbScen.name,
                description: dbScen.description ?? '',
                createdAt: new Date(dbScen.createdAt).getTime(),
                employees: Array.isArray(dbScen.employees) ? dbScen.employees : [],
                isBaseline: false,
              };
              additionalScenarios[snap.id] = snap;
            }

            set((s) => {
              s.scenarios = { [baselineSnap.id]: baselineSnap, ...additionalScenarios };
              s.activeScenarioId = baselineSnap.id;
              s.baselineScenarioId = baselineSnap.id;
              s.isSyncing = false;
            ...
            // Load org settings (pptxTheme, displaySettings etc.) — non-blocking
            fetch(`/api/org/settings?organizationId=${encodeURIComponent(organizationId)}`)
              .then((r) => r.ok ? r.json() : null)
              .then((data: { settings?: { pptxTheme?: PptxTheme; displaySettings?: DisplaySettings } } | null) => {
                if (data?.settings?.pptxTheme) {
                  set((s) => { s.pptxTheme = { ...DEFAULT_PPTX_THEME, ...data.settings!.pptxTheme }; });
                }
                if (data?.settings?.displaySettings) {
                  set((s) => { s.displaySettings = { ...DEFAULT_DISPLAY_SETTINGS, ...data.settings!.displaySettings }; });
                }
              })
              .catch(() => { /* non-critical */ });
          } catch {
        ...
        async saveNow() {
          ...
          set((st) => { st.syncStatus = 'saving'; });
          const sid = state.activeScenarioId;
          const scenario = sid ? state.scenarios[sid] : null;
          const emps = scenario?.employees ?? [];

          try {
            if (!scenario || scenario.isBaseline) {
              // Baseline: write to Employee table
              ...
              if (res.ok) {
                ...
                if (!get().autosaveEnabled) set((st) => { st.autosaveEnabled = true; });
              } else {
            ...
            } else if (scenario.dbId) {
              // Non-baseline with DB record: update Scenario table
              const res = await fetch(`/api/scenarios/${scenario.dbId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
              ...
              // Non-baseline without DB record yet: create in Scenario table
              ...
              if (res.ok) {
                ...
                set((st) => {
                  ...
                  if (sid && st.scenarios[sid]) st.scenarios[sid].dbId = data.id;
                  if (!st.autosaveEnabled) st.autosaveEnabled = true;
                });
              } else {
                const data: { error?: string } | null = await res.json().catch(() => null);
                set((st) => { st.syncStatus = 'error'; st.lastSaveError = data?.error ?? `HTTP ${res.status}`; });
    ...
    }),
    {
      ...
      migrate: (_persistedState, _version) => {
        // Any state persisted before version 2 may be stale/corrupt — start fresh.
        return {};
      },
      partialize: (s) => ({
