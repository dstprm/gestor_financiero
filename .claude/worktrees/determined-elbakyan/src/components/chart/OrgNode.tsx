...
import { getDeptColor, FLAG_META, STATUS_META } from '@/types';
...
export const OrgNode = memo(function OrgNode({ data }: NodeProps) {
  ...
  return (
    <div
      className={cn(
        'relative w-48 min-w-[140px] rounded-lg border cursor-pointer transition-all duration-150',
    ...
    >
      ...
      <div className="p-2.5">
        ...
        {/* Title */}
        <p className="text-xs text-gray-500 dark:text-slate-400 truncate leading-tight">{employee.title}</p>

        {/* Department + status */}
        <div className="flex items-center justify-between mt-1.5 gap-1">
          <span className="text-xs truncate" style={{ color: deptColor }}>
            {employee.department}
          </span>
          <span
        ...
        {/* Location */}
        {employee.location && (
          <p className="text-xs text-gray-400 dark:text-slate-500 truncate mt-0.5">📍 {employee.location}</p>
        )}
      </div>
