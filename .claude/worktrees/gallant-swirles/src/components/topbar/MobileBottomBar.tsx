...
import { DisplayOptionsPanel } from '@/components/panels/DisplayOptionsPanel';
...
export function MobileBottomBar({ onOpenSpotlight, onOpenAnalytics }: MobileBottomBarProps) {
  ...
  return (
    <>
      ...
      {moreOpen && (
        <>
          <div className="fixed inset-0 z-40 md:hidden" onClick={() => { setMoreOpen(false); setResetConfirm(false); }} />
          <div className="fixed left-0 right-0 z-50 md:hidden max-h-[80vh] overflow-y-auto bg-white dark:bg-slate-800 border-t border-gray-200 dark:border-slate-700 rounded-t-2xl shadow-xl pb-2" style={{ bottom: 'calc(4rem + env(safe-area-inset-bottom, 0px))' }}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-slate-700">
              <span className="text-sm font-medium text-gray-800 dark:text-slate-200">More</span>
              <button onClick={() => { setMoreOpen(false); setResetConfirm(false); }} className="text-gray-400 dark:text-slate-500 hover:text-gray-700 dark:hover:text-slate-300 p-1">
                <X size={16} />
              </button>
            </div>

            {/* ── View ── */}
            <p className="text-[10px] font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-widest px-4 pt-3 pb-1">View</p>

            {/* Auto Layout / Free Layout toggle */}
            <button
              onClick={() => { setLayoutMode(layoutMode === 'organized' ? 'freeStyle' : 'organized'); setResetConfirm(false); setMoreOpen(false); }}
              className="w-full flex items-center gap-3 px-4 py-3 text-sm hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors text-gray-700 dark:text-slate-300"
            >
              {layoutMode === 'freeStyle' ? <Move size={16} /> : <LayoutGrid size={16} />}
              ...
              <span className={cn(
                ...
                {layoutMode === 'freeStyle' ? 'Free' : 'Auto'}
              </span>
            </button>

            {/* Reset Free Layout — only shown in freeStyle mode */}
            {layoutMode === 'freeStyle' && (
              resetConfirm ? (
                <div className="mx-4 mb-1 flex items-center gap-2 px-3 py-2 rounded-lg bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800">
                  <span className="flex-1 text-xs text-red-700 dark:text-red-300">Reset all positions?</span>
                  <button
                    onClick={() => { resetFreeStylePositions(); setResetConfirm(false); setMoreOpen(false); }}
                  ...
                  <button
                    ...
                    className="text-xs text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-200 px-1"
                  >
                    Cancel
                  </button>
                </div>
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-500 dark:text-slate-400 hover:text-red-500 dark:hover:text-red-400 hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors"
                >
                  <RotateCcw size={15} />
            ...
            >
              ...
              <span className="flex-1 text-left">Direction</span>
            ...
            {/* Display Options */}
            <button
              onClick={() => { setMoreOpen(false); setDisplayOpen(true); }}
              className="w-full flex items-center gap-3 px-4 py-3 text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors"
            >
              <Palette size={16} />
              <span className="flex-1 text-left">Display Options</span>
            </button>

            {/* ── Tools ── */}
            <div className="border-t border-gray-100 dark:border-slate-700 mt-1" />
            <p className="text-[10px] font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-widest px-4 pt-3 pb-1">Tools</p>
            ...
            {/* Save — only when connected to DB */}
            {organizationId && (
              <div className="flex items-center gap-2 px-4 py-2">
                <button
                  onClick={() => { void saveNow(); setMoreOpen(false); }}
                  disabled={syncStatus === 'saving'}
                  className={cn(
                    'flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg border text-sm transition-colors disabled:opacity-50',
                    hasUnsavedChanges && !autosaveEnabled
                      ? 'border-amber-400 dark:border-amber-500 bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-300'
                      : 'border-gray-200 dark:border-slate-600 text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700'
                  )}
                >
                  <Save size={15} />
                  {hasUnsavedChanges && !autosaveEnabled ? 'Save (unsaved changes)' : 'Save'}
                </button>
                <button
                  ...
                  className={cn(
                    ...
                    autosaveEnabled
                      ? 'border-blue-300 dark:border-blue-600 bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-300'
                      : 'border-gray-200 dark:border-slate-600 text-gray-400 dark:text-slate-500 hover:bg-gray-100 dark:hover:bg-slate-700'
                  )}
                >
                  <Zap size={15} />
                  Auto
            ...
            >
              ...
              <span className={cn(
                ...
                {interactionMode === 'connect' ? 'Connect' : 'Navigate'}
              </span>
            </button>
            ...
            {/* ── Team ── */}
            <div className="border-t border-gray-100 dark:border-slate-700 mt-1" />
            <p className="text-[10px] font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-widest px-4 pt-3 pb-1">Team</p>
            ...
            {/* ── Preferences ── */}
            <div className="border-t border-gray-100 dark:border-slate-700 mt-1" />
            <p className="text-[10px] font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-widest px-4 pt-3 pb-1">Preferences</p>
      ...
      {/* Display options panel — mobile */}
      {displayOpen && (
        <>
          <div className="fixed inset-0 z-40 md:hidden" onClick={() => setDisplayOpen(false)} />
          <DisplayOptionsPanel onClose={() => setDisplayOpen(false)} />
        </>
      )}
