...
class DisplayOptionsPanelErrorBoundary extends Component<
...
> {
  ...
  componentDidCatch(err: unknown) {
    console.error('[DisplayOptionsPanel] Caught error:', err);
  }
  render() {
    if (this.state.hasError) {
      return (
        <div className="fixed right-4 top-14 w-72 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-xl z-50 p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-800 dark:text-slate-200">Display Options</span>
            <button onClick={this.props.onClose} className="text-gray-400 hover:text-gray-700">
              <X size={14} />
            </button>
          </div>
          <p className="text-xs text-red-500">Failed to load display options. Please try closing and reopening.</p>
        </div>
      );
    }
...
function DisplayOptionsPanelInner({ onClose }: DisplayOptionsPanelProps) {
  const rawDisplaySettings = useOrgStore((s) => s.displaySettings);
  const setDisplaySettings = useOrgStore((s) => s.setDisplaySettings);
  const saveDisplaySettings = useOrgStore((s) => s.saveDisplaySettings);
  const employees = useOrgStore(selectEmployees);

  const [openColorPicker, setOpenColorPicker] = useState<string | null>(null);

  const displaySettings = rawDisplaySettings ?? DEFAULT_DISPLAY_SETTINGS;
...
export function DisplayOptionsPanel({ onClose }: DisplayOptionsPanelProps) {
  return (
    <DisplayOptionsPanelErrorBoundary onClose={onClose}>
      <DisplayOptionsPanelInner onClose={onClose} />
    </DisplayOptionsPanelErrorBoundary>
  );
}
