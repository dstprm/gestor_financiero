...
import { Clock, Loader2, Link2, Trash2, Copy, Check, Plus, X, ArrowLeft, Pencil, CreditCard, ExternalLink, LogOut } from 'lucide-react'
...
function ActivityTab() {
  ...
  if (error) {
    return (
      <div className="py-10 text-center text-gray-500 dark:text-slate-500 text-sm">
  ...
  if (logs.length === 0) {
    return (
      <div className="py-10 text-center">
        <Clock size={28} className="mx-auto mb-3 text-gray-300 dark:text-slate-700" />
        <p className="text-gray-400 dark:text-slate-500 text-sm">No activity yet</p>
      </div>
    )
  }
    ...
    <div>
      <ul className="space-y-px">
        {logs.map((log) => (
          <li
            key={log.id}
            className="flex items-start gap-3 py-3 border-b border-gray-100 dark:border-slate-800 last:border-0"
          >
            <div className="mt-0.5 w-7 h-7 rounded-full bg-gray-100 dark:bg-slate-800 flex items-center justify-center shrink-0 text-xs font-medium text-gray-500 dark:text-slate-400">
              {log.userDisplayName[0]?.toUpperCase() ?? '?'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm text-gray-800 dark:text-slate-200">{describeAction(log)}</p>
            </div>
            <span className="text-xs text-gray-400 dark:text-slate-600 shrink-0 pt-0.5">{formatRelative(log.createdAt)}</span>
          </li>
        ))}
      </ul>
      ...
      {nextCursor && (
        <div className="pt-4 text-center">
          <button
            ...
            className="px-4 py-2 text-sm text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-slate-200 border border-gray-200 dark:border-slate-700 hover:border-gray-400 dark:hover:border-slate-500 rounded-lg transition-colors disabled:opacity-50"
          >
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={copy}
      title="Copy link"
      className="p-1 rounded text-gray-400 dark:text-slate-500 hover:text-gray-700 dark:hover:text-slate-300 transition-colors"
    >
    ...
    <div className="space-y-4">
      ...
      {showDialog && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl p-6 w-96 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-gray-900 dark:text-white font-semibold">New share link</h3>
              <button onClick={() => setShowDialog(false)} className="text-gray-400 dark:text-slate-500 hover:text-gray-700 dark:hover:text-slate-300">
                <X size={16} />
              </button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="block text-xs text-gray-500 dark:text-slate-400 mb-1">Label (optional)</label>
                <input
                  type="text"
                  value={newLabel}
                  onChange={(e) => setNewLabel(e.target.value)}
                  placeholder="e.g. Board presentation"
                  className="w-full px-3 py-2 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg text-gray-800 dark:text-slate-200 text-sm placeholder-gray-400 dark:placeholder-slate-600 focus:outline-none focus:border-blue-500"
                />
              </div>
              <div>
                <label className="block text-xs text-gray-500 dark:text-slate-400 mb-1">Expiry date (optional)</label>
                <input
                  type="date"
                  value={newExpiry}
                  onChange={(e) => setNewExpiry(e.target.value)}
                  className="w-full px-3 py-2 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg text-gray-800 dark:text-slate-200 text-sm focus:outline-none focus:border-blue-500"
                />
              </div>
            </div>
      ...
      {links.length === 0 ? (
        <div className="py-10 text-center">
          <Link2 size={28} className="mx-auto mb-3 text-gray-300 dark:text-slate-700" />
          <p className="text-gray-400 dark:text-slate-500 text-sm">No share links yet</p>
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {!link.enabled && !expired && (
                      <span className="text-xs px-1.5 py-0.5 bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-slate-500 rounded">
                        Disabled
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="text-xs text-gray-400 dark:text-slate-600 font-mono truncate max-w-[260px]">
                      {url}
                    </span>
                    <CopyButton text={url} />
                  </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      link.enabled ? 'bg-blue-600' : 'bg-gray-200 dark:bg-slate-700'
                    )}
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg text-gray-400 dark:text-slate-600 hover:text-red-500 hover:bg-red-50 dark:hover:text-red-400 dark:hover:bg-red-950/20 transition-colors"
                    title="Delete"
                  >
                    <Trash2 size={14} />
                  </button>
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/org/mine')
      .then((r) => r.ok ? r.json() : null)
      .then((d: { organizationId?: string; orgName?: string; orgs?: OrgEntry[] } | null) => {
        if (!urlOrgId && d?.organizationId) setOrgId(d.organizationId)
        if (!urlOrgId && d?.orgName) setOrgName(d.orgName)
  ...
  // Leave org state (for My Organizations tab)
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 py-10 px-4">
      ...
      <div className="max-w-2xl mx-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-5">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold text-gray-700 dark:text-slate-300">Subscription</h2>
              ...
              {subStatus ? (
                ...
                <div className="flex items-center justify-between">
                  ...
                  <a href="/billing" className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300 transition-colors">
                    Manage billing →
                  </a>
                </div>
              )}
        ...
        {activeTab === 'members' && (
          <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-5 text-center">
            <p className="text-gray-500 dark:text-slate-400 text-sm">
              Manage members on the{' '}
              <a href="/app/members" className="text-blue-600 dark:text-blue-400 hover:underline">
                Members page
              </a>
              .
            </p>
        ...
        {activeTab === 'danger' && (
          <div className="space-y-4">
            <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/40 rounded-xl p-5">
              <h2 className="text-sm font-semibold text-red-600 dark:text-red-400 mb-1">Delete organization</h2>
              <p className="text-gray-600 dark:text-slate-400 text-sm mb-4">
                This will permanently delete{' '}
                <span className="font-medium text-gray-800 dark:text-slate-300">{displayName ?? 'this organization'}</span>
                , all employees, and all data. This cannot be undone.
              </p>
              <div className="space-y-3">
                <div>
                  <label className="block text-xs text-gray-500 dark:text-slate-500 mb-1">
                    Type <span className="font-mono text-gray-700 dark:text-slate-400">{displayName}</span> to confirm
                  </label>
                  <input
                    type="text"
