...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    console.log('[PATCH /api/employees] DEBUG start', {
      organizationId,
      nodeCount: nodes.length,
      nodeIds: nodes.map((n) => n.id),
      managerIds: nodes.map((n) => ({ id: n.id, managerId: n.managerId ?? null })),
    })

    try {
      await prisma.$transaction(async (tx) => {
        // Pass 1: upsert all rows with managerId: null to guarantee every row exists
        // and clear all existing manager references (avoids FK constraint violations).
        console.log('[PATCH /api/employees] DEBUG pass1 start', nodes.length, 'nodes')
        await Promise.all(
          nodes.map((node) =>
            tx.employee.upsert({
              where: { id: node.id },
        ...
        console.log('[PATCH /api/employees] DEBUG pass1 done')

        // Pass 2: re-establish manager links only for nodes that have a managerId.
        // All rows exist now and all FKs are clear, so this is safe.
        const withManager = nodes.filter((n) => n.managerId)
        console.log('[PATCH /api/employees] DEBUG pass2 start', withManager.length, 'nodes with manager')
        if (withManager.length > 0) {
          await Promise.all(
            withManager.map((node) =>
              tx.employee.update({
                where: { id: node.id },
                data: { managerId: node.managerId },
              })
            )
          )
        }
        console.log('[PATCH /api/employees] DEBUG pass2 done')
      })
    } catch (txErr) {
      console.error('[PATCH /api/employees] DEBUG transaction failed', {
        message: (txErr as Error)?.message,
        code: (txErr as { code?: string })?.code,
        meta: (txErr as { meta?: unknown })?.meta,
        stack: (txErr as Error)?.stack,
      })
      throw txErr
    }
    ...
    console.log('[PATCH /api/employees] DEBUG soft-delete start, submittedIds:', submittedIds.length)
    try {
      ...
      console.error('[PATCH /api/employees] DEBUG soft-delete failed', {
        message: (sdErr as Error)?.message,
        code: (sdErr as { code?: string })?.code,
        meta: (sdErr as { meta?: unknown })?.meta,
        stack: (sdErr as Error)?.stack,
      })
      throw sdErr
    }
    console.log('[PATCH /api/employees] DEBUG soft-delete done')

    return NextResponse.json({ ok: true, count: nodes.length })
  } catch (err) {
    console.error('[PATCH /api/employees] ERROR', {
