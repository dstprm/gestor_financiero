...
export function TopBar({ onOpenSidebar, onOpenSpotlight, orgName, isViewer }: TopBarProps) {
  ...
  const userDisplayName = [user?.firstName, user?.lastName].filter(Boolean).join(' ') || user?.primaryEmailAddress?.emailAddress || '';
  ...
  return (
    <header className="relative h-12 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700 flex items-center px-3 gap-2 shrink-0 z-[60]">
      ...
      {/* Org switcher — hidden when search expanded on mobile */}
      <div className={cn('relative', searchExpanded && 'hidden sm:block')}>
        <button
          onClick={() => {
            const opening = !orgOpen;
            setOrgOpen(opening);
            if (opening && !orgList) {
              fetch('/api/orgs')
        ...
        >
          <Building2 size={13} className="text-gray-500 dark:text-slate-400 shrink-0" />
          <span className="font-medium text-gray-700 dark:text-slate-200 max-w-[72px] sm:max-w-[140px] truncate">
        ...
        {orgOpen && (
          <>
            ...
            <div className="absolute top-full left-0 mt-1 w-56 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-xl z-50 py-1">
              <p className="text-xs text-gray-400 dark:text-slate-500 px-3 py-1.5 uppercase tracking-wider">Switch org</p>
              {orgList ? (
                orgList.map((org) => (
                  <a
                    key={org.id}
                    ...
                    className={cn(
                      'flex items-center gap-2 px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-slate-700',
                  ...
                  >
                    ...
                    {org.id === organizationId && (
                      <span className="ml-auto text-xs text-gray-400 dark:text-slate-500">current</span>
                    )}
                  </a>
                ))
              ) : (
                <p className="px-3 py-2 text-sm text-gray-400 dark:text-slate-500">Loading…</p>
              )}
              <div className="border-t border-gray-200 dark:border-slate-700 mt-1 pt-1">
                <a
                  href="/app/orgs"
                  className="block px-3 py-2 text-sm text-gray-500 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-700 hover:text-gray-700 dark:hover:text-slate-200"
                  onClick={() => setOrgOpen(false)}
                >
                  Manage orgs →
                </a>
      ...
      {/* Filter button — always visible, primary discovery tool */}
      <button
        onClick={() => setFilterOpen((o) => !o)}
        title="Filter nodes"
        className={cn(
          'flex items-center gap-1.5 px-2.5 py-1.5 rounded-md border text-xs font-medium transition-colors shrink-0 min-h-[44px] md:min-h-0',
          hasActiveFilters
            ? 'bg-orange-50 dark:bg-orange-950/30 border-orange-400 dark:border-orange-500 text-orange-600 dark:text-orange-300'
            : 'bg-gray-100 dark:bg-slate-800 border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white hover:border-gray-400 dark:hover:border-slate-500'
        )}
      >
        <SlidersHorizontal size={13} />
        <span className="hidden sm:inline">Filters</span>
        {hasActiveFilters && <span className="w-1.5 h-1.5 rounded-full bg-orange-400 shrink-0" />}
      </button>

      {/* Auto / Free layout toggle — always visible */}
      <button
        onClick={() => setLayoutMode(layoutMode === 'organized' ? 'freeStyle' : 'organized')}
        title={layoutMode === 'freeStyle' ? 'Switch to Auto Layout' : 'Switch to Free Layout'}
        className={cn(
          'flex items-center gap-1.5 px-2.5 py-1.5 rounded-md border text-xs font-medium transition-colors shrink-0 min-h-[44px] md:min-h-0',
          layoutMode === 'freeStyle'
            ? 'bg-blue-50 dark:bg-blue-950/30 border-blue-400 dark:border-blue-500 text-blue-600 dark:text-blue-300'
            : 'bg-emerald-50 dark:bg-emerald-950/30 border-emerald-400 dark:border-emerald-500 text-emerald-600 dark:text-emerald-300'
        )}
      >
        {layoutMode === 'freeStyle' ? <Move size={13} /> : <LayoutGrid size={13} />}
        <span className="hidden sm:inline">{layoutMode === 'freeStyle' ? 'Free' : 'Auto'}</span>
      </button>
      ...
      <button
        ...
        className={cn(
          'flex items-center gap-1.5 px-2.5 py-1.5 rounded-md border text-xs font-medium transition-colors shrink-0 min-h-[44px] md:min-h-0',
          displayOpen
            ? 'bg-violet-50 dark:bg-violet-950/30 border-violet-400 dark:border-violet-500 text-violet-600 dark:text-violet-300'
            : 'bg-gray-100 dark:bg-slate-800 border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white hover:border-gray-400 dark:hover:border-slate-500'
        )}
      >
        <Palette size={13} />
        ...
        {moreOpen && (
          <>
            ...
            <div className="absolute top-full right-0 mt-1 w-52 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-xl z-50 py-1">
              ...
              {layoutMode === 'freeStyle' && (
                resetConfirm ? (
                  <div className="mx-2 mb-1 flex items-center gap-2 px-3 py-2 rounded-lg bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800">
      ...
      <div className="relative hidden md:block">
        <button
          ...
          className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-semibold hover:bg-blue-700 transition-colors shrink-0"
        >
          {user?.imageUrl ? (
          ...
          <>
            ...
            <div className="absolute top-full right-0 mt-1 w-52 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-xl z-50 py-1">
              <div className="px-3 py-2 border-b border-gray-100 dark:border-slate-700">
                <p className="text-sm font-medium text-gray-800 dark:text-slate-200 truncate">{userDisplayName}</p>
                <p className="text-xs text-gray-400 dark:text-slate-500 truncate">{user?.primaryEmailAddress?.emailAddress}</p>
              </div>
              <a
